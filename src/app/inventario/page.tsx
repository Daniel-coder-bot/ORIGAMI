  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Dialog open={openDialog} onOpenChange={(val) => {
            setOpenDialog(val)
            if(!val) { setEditandoId(null); resetForm(); }
          }}>
            <DialogTrigger asChild>
              <Button className="bg-primary hover:bg-primary/90 shadow-lg font-bold h-12 px-8 rounded-2xl">
                <Plus strokeWidth={2.5} className="mr-2 h-5 w-5" /> Nuevo Material
              </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[550px] w-[95vw] rounded-3xl border-none shadow-2xl overflow-y-auto max-h-[90vh]">
              <DialogHeader>
                <DialogTitle className="text-primary font-bold text-xl">{editandoId ? 'Editar Material' : 'Registrar Material'}</DialogTitle>
                <DialogDescription className="font-medium text-xs md:text-sm">Introduce los datos técnicos para actualizar el catálogo.</DialogDescription>
              </DialogHeader>
              <div className="grid gap-5 py-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    <Input value={formArticulo.codigo} onChange={(e) => setFormArticulo({...formArticulo, codigo: e.target.value})} className="h-12 rounded-xl font-bold" placeholder="MAT-001" />
                  </div>
                  <div className="grid gap-2">
                    <Label className="font-bold text-[10px] uppercase tracking-wider text-muted-foreground">NOMBRE DEL MATERIAL</Label>
                    <Input value={formArticulo.nombre} onChange={(e) => setFormArticulo({...formArticulo, nombre: e.target.value})} className="h-12 rounded-xl" placeholder="Ej: Tubo PVC" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="grid gap-2">
                    <Label className="font-bold text-[10px] uppercase tracking-wider text-muted-foreground">UNIDAD DE MEDIDA</Label>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Textarea value={formArticulo.descripcion} onChange={(e) => setFormArticulo({...formArticulo, descripcion: e.target.value})} className="min-h-[100px] rounded-xl resize-none p-4" placeholder="Especificaciones del material..." />
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {loading ? (
          <div className="col-span-full py-20 text-center font-black text-muted-foreground animate-pulse tracking-widest uppercase text-xs">Sincronizando existencias...</div>
          ...
          <Card key={articulo.id} className="group hover:shadow-xl transition-all duration-300 border-none bg-white overflow-hidden rounded-[2rem] ring-1 ring-primary/5">
            ...
            <CardContent className="px-8 pb-8 space-y-5">
              <p className="text-xs text-muted-foreground/80 line-clamp-2 leading-relaxed h-8 font-medium italic">{articulo.descripcion || "Sin descripción registrada."}</p>
              ...
              <div className="pt-5 border-t border-primary/5 flex items-center justify-between">
                <div className="flex flex-col">
                  ...
                  <div className="flex items-baseline gap-1.5">
                    <span className={`text-3xl font-black ${articulo.stockActual <= articulo.stockMinimo ? 'text-destructive' : 'text-primary'}`}>
          ...
          <div className="col-span-full flex flex-col items-center justify-center py-20 bg-white rounded-[3rem] border-2 border-dashed border-primary/10 opacity-50">
            <Box strokeWidth={1} className="h-24 w-24 text-muted-foreground mb-4" />
            <h3 className="text-2xl font-black text-muted-foreground uppercase tracking-tight">No se encontraron artículos</h3>
            <p className="text-xs font-black uppercase tracking-[0.2em] mt-2">Prueba ajustando los filtros</p>
          </div>
        )}
      </div>
    </div>
